...
def build_base():
    ...
    try:
        ...
        # Fallback: rectangular recess marking where branding goes
        brand_recess = (
            cq.Workplane("XY")
...
print(f"             {MEETING_W:.1f}x{MEETING_D:.1f}mm (top of base / meeting line)")
...
print()
print("--- Base Internals ---")
print(f"Reservoir:   wet zone left of X={DIVIDER_X}mm divider, depth={RESERVOIR_DEPTH:.1f}mm")
print(f"Atomizer:    {ATOMIZER_MOUNT_DIA}mm mount at ({ATOMIZER_POS_X}, {ATOMIZER_POS_Y})")
print(f"Pumps:       {PUMP_COUNT}x peristaltic, {PUMP_SPACING}mm spacing, {PUMP_BODY_W}x{PUMP_BODY_D}x{PUMP_BODY_H}mm each")
print(f"Electronics: ESP32 ({ESP32_W}x{ESP32_D}mm), 5x MOSFETs ({MOSFET_W}x{MOSFET_D}mm), PD trigger ({PD_TRIGGER_W}x{PD_TRIGGER_D}mm)")
print(f"USB-C port:  {USBC_PORT_W}x{USBC_PORT_H}mm (rear wall)")
print(f"Hex mesh:    {HEX_CELL_SIZE}mm cells, {HEX_WALL}mm walls (right + front panels)")
print(f"LED channel: {LED_CHANNEL_W}x{LED_CHANNEL_D}mm (behind hex mesh)")
...
print(f"Hatch:       {HATCH_W}x{HATCH_H}mm bottle access (front face)")
print()
print("--- Connections ---")
print(f"Magnets:     {MAGNET_COUNT}x {MAGNET_DIA}mm dia x {MAGNET_H}mm at {magnet_positions}")
print(f"Pins:        {PIN_COUNT}x {PIN_DIA}mm dia x {PIN_H}mm at {pin_positions}")
print(f"Panel lines: base Z={PANEL_LINE_Z_BASE}mm, top Z={PANEL_LINE_Z_TOP}mm ({PANEL_LINE_WIDTH}mm wide, {PANEL_LINE_DEPTH}mm deep)")
print()
